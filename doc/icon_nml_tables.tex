...
run\_nml/iforcing = 2 \tabularnewline
%
aes\_phy\_config(jg)\% lparamcpl & L & .TRUE. & &
.FALSE. : parameterizations update the physics state \newline
        so that all params get the same input state &
run\_nml/iforcing = 2 \tabularnewline
%
aes\_phy\_config(jg)\% ldrymoist & L & .FALSE. & &
.true. : use dry air mass as conserved reference air mass &
run\_nml/iforcing = 2 \tabularnewline
%
...
aes\_phy\_config(jg)\% dt\_rad > 0.000s \tabularnewline
%
aes\_rad\_config(jg)\% rrtmgp\_columns\_chunk & I & nproma & &
Column chunk size; cannot be bigger than nproma &
aes\_phy\_config(jg)\% dt\_rad > 0.000s \tabularnewline
%
...
kg m-2 & iforcing = inwp & 2d & totprec\_d\_interval & [1], [5], [6]
\tabularnewline
%
...
\item[{[5]}] \texttt{/src/atm\_phy\_nwp/mo\_nwp\_gscp\_interface}
\item[{[6]}] \texttt{/src/atm\_phy\_nwp/mo\_nwp\_diagnosis}
\end{itemize}
%
...
rain\_cmu0 &
R&  6.0 & &
Parameter of the $\mu$-$D$-relation in the rain size distribution for evaporation and sedimentation:\\
