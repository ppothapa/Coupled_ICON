...
CONTAINS
  ...
  !!
  SUBROUTINE init_wave_phy(p_patch, wave_config, p_prog, p_diag)
    ...
    CALL FETCH_LAW(p_patch, wave_config, p_diag)
    ...
    CALL init_wave_spectrum(p_patch, wave_config, p_prog, p_diag)
  ...
  !!
  SUBROUTINE init_wave_spectrum(p_patch, wave_config, p_prog, p_diag)
    CHARACTER(len=MAX_CHAR_LENGTH), PARAMETER ::  &
      ...
      DO jf = 1,wave_config%nfreqs
        DO jd = 1,wave_config%ndirs
          ...
          DO jc = i_startidx, i_endidx
            st = rpi_2*MAX(0._wp, COS(wave_config%dirs(jd)-p_diag%dir_10m(jc,jb)*deg2rad) )**2
            IF (st < 0.1E-08_wp) st = 0._wp

            ! WAM initialisation
            p_prog%tracer(jc,jk,jb,jt) = p_diag%ET(jc,jb,jf) * st
            p_prog%tracer(jc,jk,jb,jt) = MAX(p_prog%tracer(jc,jk,jb,jt),EMIN)
          END DO  !jc
        END DO  !jd
      END DO  !jf
    END DO  !jb
  ...
  !! Vectorization by Daniel Reinert, DWD (2023-01-31)
  !!
  SUBROUTINE FETCH_LAW (p_patch, wave_config, p_diag)
    ...
    DO jb = i_startblk, i_endblk
      CALL get_indices_c( p_patch, jb, i_startblk, i_endblk,           &
           &                 i_startidx, i_endidx, i_rlstart, i_rlend)
      DO jc = i_startidx, i_endidx
        IF (p_diag%sp_10m(jc,jb).GT.0.1E-08_wp) THEN
          UG = grav / p_diag%sp_10m(jc,jb)
          p_diag%FP(jc,jb) = MAX(0.13_wp, A*((grav*fetch)/(p_diag%sp_10m(jc,jb)**2))**D)
