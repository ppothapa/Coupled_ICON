!>
!! @brief Elemental subroutine for saturation adjustment at constant volume
!!
!! @author Marco Giorgetta, MPI-M, 2020-01
!!
!! @par Revision History
!!
!! This elemental code is derived from the satad_v_3d code of module mo_satad,
...
!!
!! This elemental code is designed for ICON only, to explore ways to
...
CONTAINS
  ...
  ELEMENTAL SUBROUTINE satad_v_el (rho, ta, qv, qc)
    ...
    alvocvd = ( alv + (cpv-clw)*(ta-tmelt) - rv*ta )/cvd
    twork   = ta - alvocvd*qc
    qsat    = qsat_rho(twork,rho)
    ...
    ELSE
       ...
       DO WHILE (ABS(twork-tworkold)>tol .AND. count<maxiter)
          ...
          qsat     = qsat_rho(twork,rho)
          ! Newton:
          ft    = twork - ta + alvocvd*(qsat-qv)
          dft   = 1.0_wp     + alvocvd*dqsatdt_rho(qsat,twork,rho)
          twork = twork - ft/dft;
          count = count + 1
       END DO
       !
       ...
       ta = twork
       qv = qsat
       qc = MAX(qw-qsat,zqwmin)

    END IF

  END SUBROUTINE satad_v_el

END MODULE mo_satad_v_el
