...
CONTAINS
  ...
  SUBROUTINE cover ( jg                                & !in
    ...
    ! Shortcuts to components of aes_cov_config
    !
    INTEGER :: nex, icov, jksinv, jkeinv
    REAL(wp):: csatsc, csat, crt, crs, cinv, cqx, clcon
    !
    icov   = aes_cov_config(jg)% icov
    jksinv = aes_cov_config(jg)% jksinv
    jkeinv = aes_cov_config(jg)% jkeinv
    csatsc = aes_cov_config(jg)% csatsc
    csat   = aes_cov_config(jg)% csat
    crs    = aes_cov_config(jg)% crs
    crt    = aes_cov_config(jg)% crt
    nex    = aes_cov_config(jg)% nex
    cinv   = aes_cov_config(jg)% cinv
